...
import { projectSelector,createProject, updateProject } from "../../../../api/project";
import storage from "../../../shared/storage";
import { useEffect } from "react";



export default function Attachements({info,data}){
    ...
    // useEffect(()=>{
    //     form.setFieldsValue({
    //         attachements:data?.attachements.map((item)=>{
    //             return <p>{item}</p>
    //         }),
    //     })
    // },[])
    ...
    const onFinishProject = (value) => {
        ...
        dispatch(updateProject(data._id,finishData))
